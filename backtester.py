...
try:
    ...
    print(
        "Error: Could not import bot modules during initial attempt. Adjusting sys.path for fallback."
    )
...
class SimpleBacktester:
    ...
    def __init__(
        ...
        if "risk_per_trade_percentage" not in self.config:
            self.logger.warning(
                "risk_per_trade_percentage not in config, defaulting to 1.0 for backtester."
            )
        ...
        self.logger.info(
            f"SimpleBacktester initialized for {self.symbol}. Initial Capital: {self.initial_capital_usdt} USDT. Taker Fee: {self.taker_fee_rate * 100}%"
        )
        ...
        if self.historical_data_df.empty:
            ...
            self.logger.info(
                f"Historical data loaded: {len(self.historical_data_df)} bars, from {self.historical_data_df.index.min()} to {self.historical_data_df.index.max()}"
            )

    def _get_entry_signal(
        self, current_bar_data: pd.Series, prev_bar_data: pd.Series
    ) -> str | None:
        """
        ...
        """
        if not all(k in current_bar_data for k in ["SMA_10", "SMA_20"]) or not all(
            k in prev_bar_data for k in ["SMA_10", "SMA_20"]
        ):
            # self.logger.debug("SMA_10 or SMA_20 not available in bar data. No signal.")
            return None
        if (
            pd.isna(current_bar_data["SMA_10"])
            or pd.isna(current_bar_data["SMA_20"])
            or pd.isna(prev_bar_data["SMA_10"])
            or pd.isna(prev_bar_data["SMA_20"])
        ):
        ...
        if (
            prev_bar_data["SMA_10"] <= prev_bar_data["SMA_20"]
            and current_bar_data["SMA_10"] > current_bar_data["SMA_20"]
        ):
            return "LONG"
    ...
    def run(self):
        if self.historical_data_df.empty or len(self.historical_data_df) < 2:
            self.logger.error(
                "Not enough historical data to run backtest (need at least 2 bars for crossover logic)."
            )
            return
        self.logger.info(
            f"Starting backtest run for {self.symbol} with {len(self.historical_data_df)} bars..."
        )
        ...
        for i in range(1, len(self.historical_data_df)):  # Start from the second bar
            ...
            if self.current_position_qty > Decimal("0"):
                ...
                if self.current_position_side == "LONG":
                    if (
                        current_bar["low"] <= self.current_stop_loss_price
                    ):  # Check if low breached SL
                        exit_reason = "STOP_LOSS"
                        exit_price = self.current_stop_loss_price
                    elif (
                        current_bar["high"] >= self.current_take_profit_price
                    ):  # Check if high breached TP
                        exit_reason = "TAKE_PROFIT"
                        exit_price = self.current_take_profit_price
                elif self.current_position_side == "SHORT":
                    if (
                        current_bar["high"] >= self.current_stop_loss_price
                    ):  # Check if high breached SL
                        exit_reason = "STOP_LOSS"
                        exit_price = self.current_stop_loss_price
                    elif (
                        current_bar["low"] <= self.current_take_profit_price
                    ):  # Check if low breached TP
                        exit_reason = "TAKE_PROFIT"
                        exit_price = self.current_take_profit_price

                if exit_reason:
                    entry_fee = self._calculate_fees(
                        self.current_position_qty, self.current_position_entry_price
                    )
                    exit_fee = self._calculate_fees(
                        self.current_position_qty, exit_price
                    )
                    total_fees = entry_fee + exit_fee

                    pnl_per_unit = (
                        (exit_price - self.current_position_entry_price)
                        if self.current_position_side == "LONG"
                    ...
                    self.logger.info(
                        f"{current_timestamp} - EXIT {self.current_position_side} {self.symbol} at {exit_price:.4f} due to {exit_reason}. Qty: {self.current_position_qty}. "
                    ...
                    self.trades_history.append(
                        {
                            ...
                            "qty": float(self.current_position_qty),
                            "gross_pnl": float(gross_pnl),
                            "net_pnl": float(net_pnl),
                            "total_fees": float(total_fees),
                            "exit_reason": exit_reason,
                            "balance_after_trade": float(self.balance_usdt),
                        }
                    )
                    self.current_position_qty = Decimal("0")
            ...
            if self.current_position_qty == Decimal("0"):  # Check for new entry
                ...
                if signal:
                    ...
                    sl_percentage = Decimal(
                        str(self.config.get("stop_loss_percentage", "0.02"))
                    )  # e.g. 2% SL from entry
                    if signal == "LONG":
                        stop_loss_price_hypothetical = float(
                            entry_price * (Decimal("1") - sl_percentage)
                        )
                    else:
                        stop_loss_price_hypothetical = float(
                            entry_price * (Decimal("1") + sl_percentage)
                    ...
                    if position_qty_str:
                        ...
                        self.current_stop_loss_price = Decimal(
                            str(stop_loss_price_hypothetical)
                        )
                        tick_size = self.instrument_info["priceFilter"]["tickSize"]
                        rounding_sl = (
                            ROUND_UP if signal == "LONG" else ROUND_DOWN
                        )  # Make SL price slightly worse for us
                        self.current_stop_loss_price = (
                            self.risk_manager._adjust_to_step(
                                self.current_stop_loss_price, tick_size, rounding_sl
                            )
                        )
                        ...
                        if tp_price_str:
                            ...
                            risk_reward_ratio = Decimal(
                                str(self.config.get("risk_reward_ratio", "1.5"))
                            )
                            price_diff_to_sl = abs(
                            ...
                            self.logger.warning(
                                f"Could not calculate TP based on fee coverage for {self.symbol}. Using R:R based TP: {self.current_take_profit_price:.4f}"
                            )
                        ...
                        self.logger.info(
                            f"{current_timestamp} - ENTRY {signal} {self.symbol} at {self.current_position_entry_price:.4f}. Qty: {self.current_position_qty}. "
    ...
    def calculate_performance(self):
        ...
        average_net_pnl_per_trade = (
            total_net_pnl / total_trades if total_trades > 0 else 0
        )
        total_fees_paid = sum(trade["total_fees"] for trade in self.trades_history)
        final_balance = float(
            self.balance_usdt
        )  # Balance already reflects PNL from closed trades
...
if __name__ == "__main__":
    ...
    try:
        ...
        example_config_file = os.path.join(
            project_root_path, "configs", "config.json.example"
        )

        config_to_load = (
            dev_config_file if os.path.exists(dev_config_file) else example_config_file
        )

        if not os.path.exists(config_to_load):
            logger.error(
                f"No config file found at {dev_config_file} or {example_config_file}. Exiting."
            )
            exit()
    ...
    )  # Fallback R:R if fee-based TP fails
    ...
    try:
        ...
        logger.error(
            f"Error: Data file '{data_file}' not found. Assumed in project root: {os.path.join(project_root_path, data_file)}"
        )
        exit()
    except Exception as e:
    ...
    for index, row in raw_df.iterrows():
        kline_list_from_csv.append(
            [
                ...
                str(row["close"]),
                str(row["volume"]),
                str(Decimal(str(row["open"])) * Decimal(str(row["volume"]))),
            ]
        )

    historical_df_processed = data_handler.process_kline_data(
        kline_list_from_csv, "BTCUSDT_Backtest"
    )
    ...
    historical_df_with_ta = data_handler.calculate_technical_indicators(
        historical_df_processed, "BTCUSDT_Backtest"
    )
    ...
    if len(historical_df_with_ta) < min_periods_for_signal:
        logger.error(
            f"Not enough data to form indicators requiring {min_periods_for_signal} periods. Have {len(historical_df_with_ta)}. Exiting."
        )
        exit()
    ...
    if historical_df_with_ta.empty:
        logger.error(
            f"DataFrame became empty after dropping NaNs for indicators from {len(raw_df)} initial rows. Ensure sufficient data for indicator warmup."
        )
        exit()
    logger.info(
        f"Historical data processed. Shape after TA and NaN drop: {historical_df_with_ta.shape}"
    )
