...
class RiskManager:
    ...
    def __init__(self, config: dict, config_logging: bool = True):
        ...
        if config_logging and not logging.getLogger().hasHandlers():
            ...
            self.logger.info(
                "Basic logging configured for RiskManager standalone usage."
            )

        self.config = config
        if "risk_per_trade_percentage" not in self.config:
            self.logger.warning(
                "'risk_per_trade_percentage' not found in config. Using default 1.0%."
            )
            self.config["risk_per_trade_percentage"] = 1.0

        self.logger.info(
            f"RiskManager initialized. Risk per trade: {self.config['risk_per_trade_percentage']}%"
        )
    ...
    def _adjust_to_step(
        self, value: Decimal, step_str: str, rounding_mode=ROUND_DOWN
    ) -> Decimal:
    ...
    def calculate_position_size(
        ...
        """
        self.logger.info(
            f"Calculating position size for {symbol} ({side}): Balance={available_balance_usdt}, Entry={entry_price}, SL={stop_loss_price}"
        )
        ...
        if side == "Buy":
            if entry_price <= stop_loss_price:
                self.logger.error(
                    f"For a BUY order, entry price ({entry_price}) must be greater than stop loss ({stop_loss_price})."
                )
                return None
            price_diff_per_unit = Decimal(str(entry_price)) - Decimal(
                str(stop_loss_price)
            )
        elif side == "Sell":
            if entry_price >= stop_loss_price:
                self.logger.error(
                    f"For a SELL order, entry price ({entry_price}) must be less than stop loss ({stop_loss_price})."
                )
                return None
            price_diff_per_unit = Decimal(str(stop_loss_price)) - Decimal(
                str(entry_price)
            )
        else:
        ...
        if price_diff_per_unit <= Decimal("0"):  # Should be caught by above checks too
            self.logger.error(
                f"Price difference for SL calculation is zero or negative for {symbol}, cannot size position."
            )
            return None

        risk_percentage = Decimal(
            str(self.config.get("risk_per_trade_percentage", 1.0))
        ) / Decimal("100")
        risk_amount_usdt = Decimal(str(available_balance_usdt)) * risk_percentage
        self.logger.info(
            f"Risk per trade: {self.config['risk_per_trade_percentage']}%, Risk amount (USDT): {risk_amount_usdt:.4f}"
        )

        # Initial quantity in base currency
        calculated_qty_base = risk_amount_usdt / price_diff_per_unit
        self.logger.info(
            f"Calculated initial quantity for {symbol}: {calculated_qty_base:.8f}"
        )
        ...
        if not lot_size_filter:
            self.logger.error(
                f"Missing 'lotSizeFilter' in instrument_info for {symbol}. Cannot adjust quantity."
            )
            return None
        ...
        if not all([min_order_qty_str, max_order_qty_str, qty_step_str]):
            self.logger.error(
                f"Missing one or more fields (minOrderQty, maxOrderQty, qtyStep) in lotSizeFilter for {symbol}."
            )
            return None
        ...
        adjusted_qty = self._adjust_to_step(
            calculated_qty_base, qty_step_str, ROUND_DOWN
        )
        self.logger.info(
            f"Quantity for {symbol} after applying qtyStep ('{qty_step_str}'): {adjusted_qty:.8f}"
        )

        if adjusted_qty < min_order_qty:
            self.logger.warning(
                f"Adjusted quantity {adjusted_qty} for {symbol} is below minOrderQty {min_order_qty}. Cannot place order with this risk/SL setup."
            )
        ...
        if adjusted_qty > max_order_qty:
            self.logger.warning(
                f"Adjusted quantity {adjusted_qty} for {symbol} exceeds maxOrderQty {max_order_qty}. Capping at maxOrderQty."
            )
            ...
            actual_risk_percent = (
                actual_risk_usdt / Decimal(str(available_balance_usdt))
            ) * Decimal("100")
            self.logger.info(
                f"Quantity capped at maxOrderQty. Actual risk: {actual_risk_usdt:.2f} USDT ({actual_risk_percent:.2f}% of balance)."
            )
    ...
    def calculate_fees_usdt(
        self,
        order_qty_base: float,
        execution_price_usdt: float,
        fee_rate: float,
        ...
        self.logger.debug(
            f"Calculated fee: {float(fee):.8f} USDT for Qty={order_qty_base}, Price={execution_price_usdt}, Rate={fee_rate}"
        )
        return float(fee)

    def calculate_take_profit_price_for_fee_coverage(
        self,
        entry_price: float,
        side: str,
        order_qty_base: float,
        ...
        """
        self.logger.info(
            f"Calculating TP for {fee_multiplier}x fee coverage for {symbol} ({side}): Entry={entry_price}, Qty={order_qty_base}, FeeRate={fee_rate_taker}"
        )

        if order_qty_base <= 0:
            self.logger.error(
                "Order quantity must be positive to calculate TP for fee coverage."
            )
            return None
        ...
        entry_fee_usdt = Decimal(
            str(
                self.calculate_fees_usdt(
                    order_qty_base, entry_price, fee_rate_taker, is_futures=True
                )
            )
        )
        ...
        self.logger.info(
            f"Estimated entry fee: {entry_fee_usdt:.4f}, Estimated exit fee: {exit_fee_usdt:.4f}, Total RT fees: {total_round_trip_fees_usdt:.4f} USDT for {symbol}"
        )

        target_profit_usdt = total_round_trip_fees_usdt * Decimal(str(fee_multiplier))
        self.logger.info(
            f"Target profit for {fee_multiplier}x fee coverage: {target_profit_usdt:.4f} USDT for {symbol}"
        )
        ...
        self.logger.info(
            f"Calculated raw target exit price for {symbol}: {target_exit_price_d:.8f}"
        )
        ...
        if not price_filter or "tickSize" not in price_filter:
            self.logger.error(
                f"Missing 'priceFilter' or 'tickSize' in instrument_info for {symbol}. Cannot adjust TP price."
            )
            return None
        ...
        adjusted_tp_price_d = self._adjust_to_step(
            target_exit_price_d, tick_size_str, rounding_mode=rounding_tp
        )
        ...
        self.logger.info(
            f"Final adjusted TP price for {symbol} ({side}) aiming for {fee_multiplier}x fee coverage: {final_tp_price_str} (TickSize: {tick_size_str})"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"BTCUSDT Long Position Size: {pos_size_btc_long}"
    )  # Expected risk: 1000 * 1.5% = 15 USDT. Qty = 15 / 300 = 0.05 BTC. Adjusted: 0.050
    ...
    print(
        f"ETHUSDT Short Position Size: {pos_size_eth_short}"
    )  # Expected risk: 15 USDT. Qty = 15 / 30 = 0.5 ETH. Adjusted: 0.50
    ...
    print(
        f"BTCUSDT Long Position Size (Too Small Risk/Diff): {pos_size_too_small}"
    )  # Risk: 1.5 USDT. Qty = 1.5 / 10 = 0.15. Min is 0.001. This should work.
    ...
    print(
        f"BTCUSDT Long Position Size (Actually Too Small): {pos_size_actually_too_small}"
    )  # Risk: 0.15 USDT. Qty = 0.15 / 1 = 0.15. Min is 0.001. This should be fine.
    ...
    print(
        f"BTCUSDT Long Position Size (Triggering Min Qty): {pos_size_calc_for_min_test_tight}"
    )
