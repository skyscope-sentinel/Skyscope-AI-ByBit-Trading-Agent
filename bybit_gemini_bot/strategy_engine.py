...
class StrategyEngine:
    ...
    def __init__(
        ...
        if config_logging and not logging.getLogger().hasHandlers():
            ...
            self.logger.info(
                "Basic logging configured for StrategyEngine standalone usage."
            )
        ...
        self.use_websockets = self.config.get(
            "use_websockets", False
        )  # Check config for WS usage

        self.logger.info(
            f"StrategyEngine initialized. Components linked. Use WebSockets: {self.use_websockets}"
        )
        self.logger.info(
            f"Trading symbols from config: {self.config.get('trading_symbols', 'Not Set')}"
        )
...
class StrategyEngine:
    ...
    def __init__(
        ...
        self.logger.info(
            f"StrategyEngine initialized. Use WebSockets: {self.use_websockets}, AI Confidence Threshold: {self.confidence_threshold}, Dry Run Mode: {self.dry_run_mode}"
        )
        self.logger.info(
            f"Trading symbols from config: {self.config.get('trading_symbols', 'Not Set')}"
        )

    def _format_kline_for_prompt(
        self, kline_df: pd.DataFrame, num_records: int = 20
    ) -> str:
        ...
        kline_df_str_index.index = kline_df_str_index.index.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def _format_positions_for_prompt(self, positions_data: dict, symbol: str) -> str:
        ...
        return (
            f"Symbol: {symbol_position.get('symbol')}, Side: {symbol_position.get('side')}, "
    ...
    def _construct_gemini_prompt(
        ...
        wallet_balance_str: str,
        open_positions_str: str,
    ) -> str:
        """
        ...
        strategy_guidelines = self.config.get(
            "gemini_strategy_guidelines",
        ...
        """
        self.logger.debug(
            f"Constructed Gemini prompt for {symbol} (snippet): {prompt[:300]}..."
        )
        return prompt.strip()
    ...
    def run_main_cycle(self, symbol: str):
        ...
        """
        self.logger.info(
            f"--- StrategyEngine: Starting Main Cycle for {symbol} (REST Polling) ---"
        )

        # a. Fetch Data
        try:
            self.logger.info(f"Fetching K-line data for {symbol}...")
            raw_klines_resp = self.bybit_connector.get_kline(
                category="linear",
                symbol=symbol,
                interval="3",
                limit=self.min_kline_data_points + 20,
            )  # Fetch more for TA warmup
            if not raw_klines_resp or raw_klines_resp.get("retCode") != 0:
                self.logger.error(
                    f"Failed to fetch K-line for {symbol}: {raw_klines_resp.get('retMsg', 'No response')}"
                )
                return
            ...
            kline_df_with_ta = self.data_handler.calculate_technical_indicators(
                kline_df, symbol
            )
            kline_df_with_ta.dropna(inplace=True)  # Remove rows with NaN indicators
            if len(kline_df_with_ta) < self.min_kline_data_points:
                self.logger.warning(
                    f"Not enough K-line data points ({len(kline_df_with_ta)}) after TA and NaN drop for {symbol} (need {self.min_kline_data_points}). Skipping cycle."
                )
                return
            kline_data_for_prompt = self._format_kline_for_prompt(
                kline_df_with_ta, num_records=self.min_kline_data_points
            )

            self.logger.info(f"Fetching ticker info for {symbol}...")
            ticker_info_resp = self.bybit_connector.get_tickers(
                category="linear", symbol=symbol
            )
            if (
                not ticker_info_resp
                or ticker_info_resp.get("retCode") != 0
                or not ticker_info_resp.get("result", {}).get("list")
            ):
                self.logger.error(
                    f"Failed to fetch ticker for {symbol}: {ticker_info_resp.get('retMsg', 'No response or empty list')}"
                )
                return
            current_price_str = ticker_info_resp["result"]["list"][0]["lastPrice"]

            self.logger.info("Fetching wallet balance for USDT...")
            balance_info_resp = self.bybit_connector.get_wallet_balance(
                account_type="UNIFIED", coin="USDT"
            )  # Assuming UNIFIED account
            if (
                not balance_info_resp
                or balance_info_resp.get("retCode") != 0
                or not balance_info_resp.get("result", {}).get("list")
            ):
                self.logger.error(
                    f"Failed to fetch USDT balance: {balance_info_resp.get('retMsg', 'No response or empty list')}"
                )
                return
            # Find the USDT balance entry
            usdt_balance_details = next(
                (
                    acc["coin"][0]
                    for acc in balance_info_resp["result"]["list"]
                    if acc.get("coin") and acc["coin"][0].get("coin") == "USDT"
                ),
                None,
            )
            if (
                not usdt_balance_details
                or "availableToWithdraw" not in usdt_balance_details
            ):  # or 'walletBalance'
                self.logger.error(
                    f"Could not find 'availableToWithdraw' for USDT in balance response: {usdt_balance_details}"
                )
                return
            available_balance_str = usdt_balance_details[
                "availableToWithdraw"
            ]  # Or use walletBalance if preferred for total equity

            self.logger.info(f"Fetching positions for {symbol}...")
            positions_info_resp = self.bybit_connector.get_positions(
                category="linear", symbol=symbol
            )
            open_positions_str = self._format_positions_for_prompt(
                positions_info_resp, symbol
            )

            self.logger.info(f"Fetching instrument info for {symbol}...")
            instrument_info_resp = self.bybit_connector.get_instrument_info(
                category="linear", symbol=symbol
            )
            if (
                ...
                self.logger.error(
                    f"Failed to fetch instrument info for {symbol}: {instrument_info_resp.get('retMsg', 'No response or empty list')}"
                )
                return
            instrument_info = instrument_info_resp["result"]["list"][0]

            self.logger.info(f"Fetching fee rates for {symbol}...")
            fee_rate_resp = self.bybit_connector.get_fee_rates(
                category="linear", symbol=symbol
            )
            if (
                not fee_rate_resp
                or fee_rate_resp.get("retCode") != 0
                or not fee_rate_resp.get("result", {}).get("list")
            ):
                self.logger.error(
                    f"Failed to fetch fee rates for {symbol}: {fee_rate_resp.get('retMsg', 'No response or empty list')}"
                )
                return
            fee_rate_info = fee_rate_resp["result"]["list"][0]
            taker_fee_rate_str = fee_rate_info.get(
                "takerFeeRate", str(self.config.get("taker_fee_rate", "0.00055"))
            )

        except Exception as e:
            self.logger.error(
                f"Error during data fetching phase for {symbol}: {e}", exc_info=True
            )
            return
        ...
        if (
            "None for" not in open_positions_str and open_positions_str != "None"
        ):  # Crude check for existing position
            self.logger.info(
                f"Existing position found for {symbol}: {open_positions_str}. Holding off on new entry based on LLM for this cycle. Implement position management logic separately."
            )
        ...
        prompt_str = self._construct_gemini_prompt(
            symbol,
            kline_data_for_prompt,
            current_price_str,
        ...
        if not analysis_result or "error" in analysis_result:
            self.logger.error(
                f"Failed to get valid analysis from Gemini for {symbol}. Result: {analysis_result}"
            )
            return

        self.logger.info(
            f"Gemini Analysis for {symbol}: {json.dumps(analysis_result, indent=2)}"
        )
        ...
        if decision in ["BUY", "SELL"] and confidence >= self.confidence_threshold:
            self.logger.info(
                f"Decision for {symbol}: {decision} with confidence {confidence:.2f} - proceeding with trade considerations."
            )

            try:
                entry_price_str = analysis_result.get("entry_price")
                stop_loss_price_str = analysis_result.get("stop_loss_price")
                take_profit_price_llm_str = analysis_result.get(
                    "take_profit_price"
                )  # TP from LLM

                if entry_price_str is None or stop_loss_price_str is None:
                    self.logger.error(
                        f"LLM provided {decision} but missing entry_price or stop_loss_price for {symbol}."
                    )
                    return
                ...
                self.logger.info(
                    f"Calculating position size for {symbol} {decision} at {entry_price} SL {stop_loss_price}..."
                )
                ...
                if qty_str and float(qty_str) > 0:
                    ...
                    tp_price_for_order_str = (
                        self.risk_manager.calculate_take_profit_price_for_fee_coverage(
                            entry_price=entry_price,
                            side=decision,
                            ...
                            fee_multiplier=float(
                                self.config.get("tp_fee_multiplier", 3.0)
                            ),
                        )
                    )
                    if not tp_price_for_order_str:  # Fallback if calculation failed
                        self.logger.warning(
                            f"RiskManager TP calculation failed for {symbol}. Attempting to use LLM TP or fallback."
                        )
                        if take_profit_price_llm_str:
                            tp_price_for_order_str = str(
                                float(take_profit_price_llm_str)
                            )  # Ensure it's string
                        else:  # Absolute fallback: SL distance based R:R
                            risk_reward_ratio = self.config.get(
                                "risk_reward_ratio", 1.5
                            )
                            price_diff_to_sl = abs(entry_price - stop_loss_price)
                            if decision == "BUY":
                                tp_fallback = entry_price + (
                                    price_diff_to_sl * risk_reward_ratio
                                )
                            else:
                                tp_fallback = entry_price - (
                                    price_diff_to_sl * risk_reward_ratio
                            ...
                            tp_price_for_order_str = str(
                                self.risk_manager._adjust_to_step(
                                    Decimal(str(tp_fallback)),
                                    instrument_info["priceFilter"]["tickSize"],
                                )
                            )

                    sl_price_for_order_str = str(
                        stop_loss_price
                    )  # Use LLM's SL directly after validation

                    self.logger.info(
                        f"Attempting to place {decision} order for {symbol}: Qty={qty_str}, EntryPx={entry_price_str}, SL={sl_price_for_order_str}, TP={tp_price_for_order_str}"
                    )

                    # Ensure all prices are strings with correct precision for API
                    # Price precision from instrument_info['priceFilter']['tickSize']
                    price_precision = self.risk_manager._get_price_precision(
                        instrument_info["priceFilter"]["tickSize"]
                    )

                    formatted_entry_price = f"{entry_price:.{price_precision}f}"
                    formatted_sl_price = (
                        f"{float(sl_price_for_order_str):.{price_precision}f}"
                    )
                    formatted_tp_price = (
                        f"{float(tp_price_for_order_str):.{price_precision}f}"
                    )

                    order_details_log = (
                        f"Symbol={symbol}, Side={decision}, Type=Limit, Qty={qty_str}, "
                        f"EntryPx={formatted_entry_price}, SL={formatted_sl_price}, TP={formatted_tp_price}"
                    )

                    if self.dry_run_mode:
                        self.logger.info(
                            f"[DRY RUN] Would place order: {order_details_log}"
                        )
                        order_response = {
                            "retCode": 0,
                            "retMsg": "OK (Dry Run)",
                            "result": {
                                "orderId": f"dryrun_{symbol}_{int(time.time())}"
                            },
                            "opInfo": "Dry Run Order - Not Sent",
                        }
                    else:
                        self.logger.info(
                            f"[LIVE RUN] Placing order: {order_details_log}"
                        )
                    ...
                    self.logger.info(
                        f"Order placement action response for {symbol}: {order_response}"
                    )
                    if order_response and order_response.get("retCode") == 0:
                        self.logger.info(
                            f"Order action successful for {decision} order for {symbol}. OrderID: {order_response.get('result', {}).get('orderId')}"
                        )
                    else:
                        self.logger.error(
                            f"Order action FAILED for {decision} order for {symbol}. Response: {order_response}"
                        )

                else:
                    self.logger.warning(
                        f"Quantity calculation failed or resulted in zero/None for {symbol}. Not placing order. Details: {qty_str}"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error processing decision and executing trade for {symbol}: {e}",
                    exc_info=True,
                )

        elif decision == "HOLD":
            self.logger.info(
                f"Decision for {symbol} is HOLD. Confidence: {confidence:.2f}. Reason: {analysis_result.get('reasoning', 'N/A')}"
            )
        else:  # NO_ACTION or other
            self.logger.info(
                f"Decision for {symbol} is {decision}. Confidence: {confidence:.2f}. No trading action taken. Reason: {analysis_result.get('reasoning', 'N/A')}"
            )

        # e. Sleep (if polling)
        if not self.use_websockets:
            sleep_duration = self.config.get(
                "polling_interval_seconds", 180
            )  # Default to 3 minutes
            self.logger.info(
                f"Polling mode: Sleeping for {sleep_duration} seconds before next cycle for {symbol}."
            )
    ...
    def process_websocket_update(self, data_type: str, data: dict, symbol: str):
        ...
        """
        self.logger.info(
            f"StrategyEngine: Received WebSocket update. Type: {data_type}, Symbol: {symbol}"
        )
        # self.logger.debug(f"Data: {data}")

        if data_type == "kline_3m_completed":
            self.logger.info(
                f"A new 3-minute kline has completed for {symbol}. Triggering analysis cycle."
            )
            ...
            self.logger.info(
                f"Orderbook updated for {symbol}. Top bid: {data.get('bids', [{}])[0] if data.get('bids') else 'N/A'}, Top ask: {data.get('asks', [{}])[0] if data.get('asks') else 'N/A'}"
            )
            pass
...
if __name__ == "__main__":
    ...
    class MockBybitConnector:
        def __init__(self, api_key, api_secret, testnet):
            self.logger = logging.getLogger("MockBybit")
    ...
    class MockGeminiAnalyzer:
        def __init__(self, api_key, model_name):
            self.logger = logging.getLogger("MockGemini")

    class MockDataHandler:
        def __init__(self):
            self.logger = logging.getLogger("MockDataHandler")
