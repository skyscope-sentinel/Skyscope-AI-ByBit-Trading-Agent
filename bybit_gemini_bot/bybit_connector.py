...
class BybitConnector:
    ...
    def __init__(
        ...
        if (
            config_logging and not logging.getLogger().hasHandlers()
            ...
            self.logger.info(
                "Basic logging configured for BybitConnector standalone usage."
            )
        ...
        ):  # HTTP session needs keys for most authenticated endpoints
            try:
                self.session = HTTP(
                ...
                self.logger.info(
                    f"BybitConnector HTTP session initialized. Testnet: {self.testnet}. API Key ending with: ...{self.api_key[-4:]}"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to initialize pybit HTTP session: {e}", exc_info=True
                )
                self.session = None
        elif self.api_key:  # Only API key might be provided for read-only operations or certain WS auth.
            self.logger.info(
                f"BybitConnector initialized with API Key only (no secret). Testnet: {self.testnet}. API Key ending with: ...{self.api_key[-4:]}"
            )
        else:
            self.logger.warning(
                "BybitConnector initialized without API key/secret. Only public unauthenticated endpoints will work."
            )
    ...
    def _make_request(self, method_name: str, **kwargs):
        ...
        if self.session is None:
            self.logger.error(
                f"Cannot make request '{method_name}'. Session not initialized (likely missing API key/secret or initialization failed)."
            )
            return None
        ...
        try:
            ...
            log_kwargs = {
                k: (v if k not in ["api_key", "api_secret"] else "********")
                for k, v in kwargs.items()
            }
            self.logger.debug(
                f"Making Bybit API request. Method: {method_name}, Params: {log_kwargs}"
            )
            ...
            if hasattr(
            ...
            ):  # pybit stores last response headers
                ...
                if rate_limit_status:
                    self.logger.debug(
                        f"Rate Limit Status: {rate_limit_status}, Limit: {rate_limit}, Reset: {rate_limit_reset}"
                    )
            ...
            if isinstance(response, dict):
                ...
                if ret_code == 0:
                    self.logger.debug(
                        f"Request '{method_name}' successful. retCode: {ret_code}"
                    )
                    return response
                else:
                ...
                self.logger.error(
                    f"Unexpected response format from '{method_name}': {type(response)}. Expected dict. Response: {response}"
                )
                return None

        except AttributeError:
            self.logger.error(
                f"Method '{method_name}' not found in pybit session.", exc_info=True
            )
            return None
        except (
            Exception
        ) as e:  # Catching generic Exception, pybit might raise specific ones
            self.logger.error(
                f"Exception during Bybit API request '{method_name}': {e}",
                exc_info=True,
            )
            return None
    ...
    def get_orderbook(
        self, category: str, symbol: str, limit: int = 1
    ):  # Default limit to 1 for top bid/ask as per Bybit doc for V5
        ...
        return self._make_request(
            "get_orderbook", category=category, symbol=symbol, limit=limit
        )
    ...
    def connect_public_ws(self, data_handler_callback_map: dict = None):
        ...
        if self.ws_public:
            self.logger.warning(
                "Public WebSocket already initialized. Disconnect first or use existing."
            )
            return

        self.logger.info(
            f"Connecting to Public WebSocket: {self.ws_url_public_linear} (using linear channel type by default)"
        )
        self.ws_public = (
            HTTP(  # Re-using HTTP for WebSocket setup as per pybit docs for v5
                testnet=self.testnet,
        ...
        self.logger.info(
            "Public WebSocket 'connection' (via HTTP session for stream setup) ready."
        )

    def subscribe_to_kline_public(
        self, symbol: str, interval_minutes: int, category: str = "linear"
    ):
        if (
            not self.ws_public
            or not self.public_ws_handlers
            or "kline" not in self.public_ws_handlers
        ):
            self.logger.error(
                "Public WebSocket or kline handler not initialized. Call connect_public_ws() first with a kline callback."
            )
            return
        topic = f"kline.{interval_minutes}.{symbol}"  # Example topic format, adjust if pybit uses different
        self.logger.info(
            f"Subscribing to public K-line for {symbol}, interval {interval_minutes}min (Category: {category})"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Successfully initiated K-line stream subscription for {symbol} via HTTP session's kline_stream."
            )
        except Exception as e:
            self.logger.error(
                f"Error subscribing to K-line stream for {symbol}: {e}", exc_info=True
            )

    def subscribe_to_orderbook_public(
        self, symbol: str, depth: int = 25, category: str = "linear"
    ):
        if (
            not self.ws_public
            or not self.public_ws_handlers
            or "orderbook" not in self.public_ws_handlers
        ):  # check ws_public (HTTP session)
            self.logger.error(
                "Public WebSocket or orderbook handler not initialized. Call connect_public_ws() first."
            )
            return
        self.logger.info(
            f"Subscribing to public Orderbook for {symbol}, depth {depth} (Category: {category})"
        )
        try:
            ...
            self.logger.info(
                f"Successfully initiated Orderbook stream subscription for {symbol} via HTTP session's orderbook_stream."
            )
        except Exception as e:
            self.logger.error(
                f"Error subscribing to Orderbook stream for {symbol}: {e}",
                exc_info=True,
            )
    ...
    def subscribe_to_public_trades(self, symbol: str, category: str = "linear"):
        if (
            not self.ws_public
            or not self.public_ws_handlers
            or "public_trade" not in self.public_ws_handlers
        ):
            self.logger.error(
                "Public WebSocket or public_trade handler not initialized. Call connect_public_ws() first."
            )
            return
        self.logger.info(
            f"Subscribing to public Trades for {symbol} (Category: {category})"
        )
        try:
            ...
            self.logger.info(
                f"Successfully initiated Public Trades stream subscription for {symbol} via HTTP session's trade_stream."
            )
        except Exception as e:
            self.logger.error(
                f"Error subscribing to Public Trades stream for {symbol}: {e}",
                exc_info=True,
            )
    ...
    def start_public_ws(self):
        ...
        if (
            not self.session
        ):  # Check if HTTP session (which manages streams) is available
            self.logger.error(
                "HTTP Session not available. Cannot start WebSocket streams."
            )
            return
        self.logger.info(
            "Public WebSocket streams (kline, orderbook, etc.) are managed by pybit's HTTP session object and run in background threads once subscribed."
        )
        self.logger.info(
            "No explicit 'run_forever()' needed for public streams when using HTTP session's stream methods."
        )
    ...
    def connect_private_ws(self, data_handler_callback_map: dict = None):
        ...
        if self.ws_private:
            self.logger.warning(
                "Private WebSocket already initialized. Disconnect first or use existing."
            )
            return

        if not self.api_key or not self.api_secret:
            self.logger.error(
                "API key and secret are required for Private WebSocket. Cannot connect."
            )
            return
        ...
        try:
            self.ws_private = (
                pybit.unified_trading.WebSocket(  # Direct WebSocket class usage
                    testnet=self.testnet,
            ...
            self.logger.error(
                f"Failed to initialize pybit Private WebSocket client: {e}",
                exc_info=True,
            )
            self.ws_private = None
    ...
    def subscribe_to_orders_private(self):
        if not self.ws_private or "orders" not in self.private_ws_handlers:
            self.logger.error(
                "Private WebSocket or order handler not initialized. Call connect_private_ws() first."
            )
            return
    ...
    def start_private_ws(self):
        ...
        self.logger.info(
            "Starting private WebSocket connection (run_forever in a new thread)..."
        )
        ...
        # ws_thread.start()
        self.logger.info(
            "Conceptual: Private WebSocket would be started in a separate thread using ws_private.run_forever()."
        )
...
if __name__ == "__main__":
    ...
    def get_test_config():
        ...
        project_root = os.path.dirname(
            script_dir
        )  # Assumes this file is in bybit_gemini_bot/
        ...
        if not os.path.exists(config_to_load):
            print(
                f"Error: No config file found at {dev_config} or {example_config}. Cannot run standalone test."
            )
            return None
        ...
        try:
            with open(config_to_load, "r") as f:
                ...
                api_secret = cfg.get("testnet_api_secret_bybit") or cfg.get(
                    "bybit_api_secret"
                )
                ...
                if (
                    ...
                    print(
                        "Warning: API keys in config are placeholders. Real API calls requiring auth will fail."
                    )
                    print(
                        f"Using Key: {api_key[:5]}...{api_key[-3:] if api_key else 'N/A'}"
                    )
    ...
    if connector.session is None:
        print(
            "Failed to initialize Bybit session. Check API keys and network. Exiting."
        )
        exit()
    ...
    kline_data = connector.get_kline(
        category="linear", symbol="BTCUSDT", interval="60", limit=5
    )
    if kline_data and kline_data.get("retCode") == 0:
        logger.info(
            f"K-line data for BTCUSDT (linear): {len(kline_data.get('result', {}).get('list', []))} candles"
        )
    ...
    if instrument_info and instrument_info.get("retCode") == 0:
        logger.info(
            f"Instrument Info for BTCUSDT (linear): Found {len(instrument_info.get('result', {}).get('list', []))} instrument(s)."
        )
        # logger.debug(json.dumps(instrument_info, indent=2))
    else:
        logger.warning(
            f"Failed to get instrument info or error in response: {instrument_info}"
        )
    ...
    if tickers_linear and tickers_linear.get("retCode") == 0:
        logger.info(
            f"Tickers for BTCUSDT (linear): {tickers_linear.get('result', {}).get('list', [])[0] if tickers_linear.get('result', {}).get('list') else 'No data'}"
        )
    else:
        logger.warning(
            f"Failed to get linear tickers or error in response: {tickers_linear}"
        )

    # Orderbook for BTCUSDT (Testnet)
    orderbook_data = connector.get_orderbook(
        category="linear", symbol="BTCUSDT", limit=5
    )
    if orderbook_data and orderbook_data.get("retCode") == 0:
        logger.info(
            f"Orderbook for BTCUSDT (linear): Ask: {orderbook_data.get('result', {}).get('a', [])[:2]}, Bid: {orderbook_data.get('result', {}).get('b', [])[:2]}"
        )
        # logger.debug(json.dumps(orderbook_data, indent=2))
    else:
        logger.warning(
            f"Failed to get orderbook data or error in response: {orderbook_data}"
        )

    logger.info(
        "--- Testing Account Data Endpoints (Testnet - Requires Valid Testnet API Keys with permissions) ---"
    )

    # Wallet Balance (UNIFIED account on Testnet)
    # Ensure your Testnet API key has "Account Information" permissions
    wallet_balance = connector.get_wallet_balance(
        account_type="UNIFIED"
    )  # UNIFIED or CONTRACT or SPOT
    if wallet_balance and wallet_balance.get("retCode") == 0:
        logger.info(
            f"Wallet Balance (UNIFIED): {json.dumps(wallet_balance.get('result', {}).get('list', [])[0] if wallet_balance.get('result', {}).get('list') else 'No balance data', indent=2)}"
        )
    else:
        logger.warning(
            f"Failed to get wallet balance or error in response (check API key permissions): {wallet_balance}"
        )
    ...
    if fee_rates and fee_rates.get("retCode") == 0:
        logger.info(
            f"Fee Rates for BTCUSDT (linear): {fee_rates.get('result', {}).get('list', [])[0] if fee_rates.get('result', {}).get('list') else 'No fee rate data'}"
        )
    else:
        logger.warning(f"Failed to get fee rates or error in response: {fee_rates}")

    logger.info(
        "--- Example Order Placement & Management (Testnet - Requires Trading Permissions & Funds) ---"
    )
    logger.warning(
        "Order placement tests are commented out by default to prevent accidental orders."
    )
    logger.warning(
        "Uncomment and ensure your Testnet account has USDT funds to test these."
    )
