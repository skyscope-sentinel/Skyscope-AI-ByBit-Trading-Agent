...
def setup_logging(
    ...
    if not os.path.exists(LOG_DIR):
        try:
            ...
            logging.error(
                f"Could not create log directory. File logging disabled. Error: {e}"
            )
            return
    ...
    try:
        ...
        logging.error(
            f"Failed to set up file logging for '{log_file}': {e}. Logging to console only."
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DEFAULT_LOG_FILE):
        print(
            f"Log file '{DEFAULT_LOG_FILE}' was NOT created (this is unexpected if no errors occurred)."
        )
    else:
